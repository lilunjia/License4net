...
class AuthorizeFile
{
    ...
    public bool Check()
    {
        ...
        if (this.AuthorizeType.Equals("网址授权"))
        {
            var siteUrl = HttpContext.Current.Request.Url.AbsolutePath;
            foreach (string item in items)
            {
                if (siteUrl.ToLower().Contains(item.ToLower()))
                {
                    return true;
                }
            }
        }
        else if (this.AuthorizeType.Equals("CPU序列号"))
        {
